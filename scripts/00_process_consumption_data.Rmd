...
# Plan
1. Create a data frame where each row is a sampled predator. Include predator specific information (weight, species), tow specific information (towid, season, location, depth, year). This approach doesn't account for the correlation of predators within tows.
...
save_data <- TRUE
use_all_clupea <- TRUE   # if TRUE use all clupea spp. otherwise use only C. harengus
use_empty_stomachs <- TRUE # include predators whose stomachs were empty?
```

```{r load-raw-diet-data}
frdata <- read_csv(here("data", "raw", "fr_diet.csv"), guess_max = 365080) %>%
  dplyr::select(-X1)
```
...
## Prey Data
I will not use eggs, larvae, or scales, but like Deroba 2019, I will use the Clupidae category, since these are the most common clupeidae. All prey weights are real numbers. 
...
Version (1) Exclude predators whose stomachs were empty, because we don't know why they were empty (e.g., regurgitated in trawl due to barotrauma).

Version (2) Empty stomachs are data, include those too. 
```{r simplify-predator-data}
if(use_empty_stomachs == FALSE){
  preddat <- frdata %>%
    ...
    distinct()
}else if(use_empty_stomachs == TRUE){
  preddat <- frdata %>%
    dplyr::select(towid, station, geoarea, declat, declon, month, day, year, season, setdepth,
                  predid, pdcomnam, pdsex, pdlen, sizecat, pdwgt) %>%
    distinct()
...
Now join with data frame with the rest of the predator data. It should have the same number of rows as predid unless I have wonky duplicated stuff that needs more investigation. Need to convert NAs to 0s for unobserved prey. 
...
# 4. Rename and reformat outputs
```{r format-data}
dat_preds_all <- dat_preds_all %>%
  select(-season) %>%
  rename(
    season = myseason,
    predator = pdcomnam
...
```{r save-output}
if(save_data == TRUE){
  saveRDS(dat_preds_all, here("data", "processed", "dat_preds_all.rds"))
  saveRDS(dat_tows_all, here("data", "processed", "dat_tows_all.rds"))
}
```
