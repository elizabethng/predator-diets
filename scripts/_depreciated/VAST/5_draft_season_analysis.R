...
DateFile = paste("output", "VAST", paste0("season_PLG_", Species_name), sep = "/")
...
dat = read_rds(here("output", "data_formatted", "dat_preds.rds")) %>%
  ...
  mutate(season_fac = as.numeric(season_fac))
...
FieldConfig = c(
  "Omega1"   = 4,   # number of spatial variation factors (0, 1, AR1)
  "Epsilon1" = 4,   # number of spatio-temporal factors
  "Omega2"   = 4, 
  "Epsilon2" = 4) 
