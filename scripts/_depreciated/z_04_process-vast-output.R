...
modruns <- readr::read_rds(here::here("output", "raw_trawl.rds"))
...
write_csv(failed, here::here("output", "failed_trawl.csv"))
# All the white hake spring, but that makes sense because there was very little data for those runs
...
topmods <- worked %>%
  dplyr::group_by(species, season) %>%
  dplyr::top_n(n = -1, wt = aic)
readr::write_rds(topmods, here::here("output", "top_trawl.rds"))
