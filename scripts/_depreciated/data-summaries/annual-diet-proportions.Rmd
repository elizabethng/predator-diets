---
title: "Annual diet proportions by predator age and prey species"
output: html_notebook
editor_options: 
  chunk_output_type: inline
---

Based on 01_process_consumption_data.Rmd, but I need to keep more prey items.
...
# Note:
Need to look at relative consumption, so at minimum, a category for herring consumed, that predator consumed, and the amount of other prey consumed, whatever that should include. Maybe go by gut volume since that's the most straight forward? But what to do about "other prey" that aren't actually food (e.g., "STRING", "PLASTIC"), and non-fish prey? Non-fish prey seem important and should be inluded too I think. I also doubt that the non-food categories are very significant. 

Check consumption of non-food items.
```{r}
non_food <- frdata %>% 
  filter(analsci == "MISCELLANEOUS") %>%
  select(pynam) %>%
  distinct() %>%
  pull(pynam)

filter(frdata, analsci == "MISCELLANEOUS") %>%
  group_by(year) %>%
  summarise(
    tot_mass = sum(pyamtw),
    tot_volm = sum(pyamtv),
