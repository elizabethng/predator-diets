# Fit index standardization model to pooled (tow-level) dogfish consumption data
...
DateFile = paste("output", "VAST", "reading-group", "dogfish-consumption_pooled_plg_option-4c", sep = "/")
...
if(FALSE){
  dat %>% 
    group_by(year) %>% 
    summarise(tot_cons = sum(mean_pyamtw)) %>% 
    filter(tot_cons == 0)
  
  # need to set 1980 to NA
  dat = dat %>%
    mutate(mean_pyamtw = replace(mean_pyamtw, year == 1980, NA))
}
...
# Check the knot locations (spare southern region)
if(FALSE){
...
FieldConfig = c(
  ...
  "Omega2"   = 1, 
  "Epsilon2" = 1) 
# IID same as 1 for single-species models?
