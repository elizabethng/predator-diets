---
title: "NEFSC Food Habits Database: Exploratory Data Analysis"
author: "Elizabeth Ng"
date: "August 16, 2018"
...
localfile = c("D:/Dropbox/Predator Diets/")
```


Load the data, omitting the first column, which is simply a unique ID from the database (I assume).  Also note, they are not consecutive entries probably because Brian just pulled out the top 14 herring predators.  I needed to sex the max lines read, since I was getting errors for inconsistent types within columns (note: I could use `problems(dataframe)` or add `tokenizer_csv()` to the read statement to check that out).
```{r load data, include = FALSE}
frdata = read_csv(paste0(localfile, "data/fr_diet.csv"), guess_max = 365080) %>%
  select(-X1)
...
There are `{r} frdata %>% count` rows, which represent individual items/species of prey in each predator's stomach. 

There are `{r} frdata %>% count(cruise6) %>% count` cruises (should be one per season per year). 
